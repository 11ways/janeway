module.exports = function defineLogList(Janeway, Blast, Bound) {
	 ...
	 */
	var LogList = F.inherits(function LogList(screen, box) {
	...
	LogList.setMethod(function pushLine(line, defer) {
		...
		if (!defer) {
			this.box.setLine(line.index, line.toString());
			return;
		}

		setImmediate(function deferAndRenderLine() {
			that.box.setLine(line.index, line.toString());
			that.render();
		});
	});
	...
	LogList.setMethod(function insertAfter(line, afterIndex) {
		...
		this.box.insertLine(index, line.toString());
	});
	 ...
	 * @version  0.1.0
	 */
	LogList.setMethod(function render() {
		this.box.render();
		this.screen.render();
	});
	 ...
	 */
	LogList.setMethod(function consoleLog(args) {
