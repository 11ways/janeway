...
JanewayClass.setMethod(function _initLeftHover() {
	...
	this.screen.on('element mouseover', function onMouseOver(el, data) {
		...
		if (!el._janeway_indicator) {
			return;
		}

		el._janeway_indicator.onMouseOver(data);
	});

	this.screen.on('element mouseout', function onMouseOut(el, data) {

		if (!el._janeway_indicator) {
			return;
		}

		el._janeway_indicator.onMouseOut(data);
	});
