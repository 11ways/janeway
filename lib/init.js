 ...
 * @return   {Janeway.Sta}
 */
JanewayClass.setMethod(function setStatus(text) {

	var status = new Janeway.Status(this, text);
	...
	cli.on('keypress', function onKeypress(e, key) {
		...
		if (key.name == 'pagedown') {
			...
			try {
				...
				try {
					result = vm.runInNewContext(cmd, vm_context);
				} catch (err) {
					// In case it failed, it might have been an expression,
					// so try the same code inside parentheses
					result = vm.runInNewContext('(' + cmd + ')', vm_context);
				}
