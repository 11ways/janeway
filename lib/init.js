...
JanewayClass.setMethod(function print(type, args, options) {
	...
	if (this.logList) {
		return this.logList.consoleLog(args, type, options);
	} else {
...
JanewayClass.setMethod(function _getUserConfig() {
	...
	conf_path = libpath.resolve(this.user_janeway_path, 'janeway.json');
...
JanewayClass.setMethod(function saveCliHistory() {
	...
	try {
		fs.writeFile(this.history_path, JSON.stringify(data, null, 2));
	} catch (err) {
		// Ignore
	}
});
...
JanewayClass.setMethod(function start(options, callback) {
	...
	if (!Blast.isNW) {
		...
		setTimeout(function() {
		console.log('Setting output:', screen);
	},50)
