...
JanewayClass = Blast.Collection.Function.inherits('Informer', 'Develry.Janeway', function Janeway() {
	...
	this.vm_context = vm.createContext(vm_context);
...
/**
 * Symbol for a method to be used to get a custom instance identifier of an argument
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.4.3
 * @version  0.4.3
 */
JanewayClass.setStatic('ARG_LEFT', Symbol.for('janeway_arg_left'));

/**
 * Symbol for a method to be used to get the second string representation of an argument
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.4.3
 * @version  0.4.3
 */
JanewayClass.setStatic('ARG_RIGHT', Symbol.for('janeway_arg_right'));
 ...
 * @version  0.4.3
 */
JanewayClass.setProperty(function has_started() {
	return !!this.screen;
});
...
try {
	...
	// Make sure the original write methods are set
...
JanewayClass.setMethod(function setTitle(title) {
	...
	// Don't set terminal title in non-TTY mode (escape sequences won't be interpreted)
 ...
 * @param    {String}  text
 *
 * @return   {Boolean}   True if the sequence was written
 */
JanewayClass.setMethod(function writeToClipboard(text) {

	// Don't write in non-TTY mode (escape sequences won't be interpreted)
	if (!process.stdout.isTTY) {
		return false;
	}
	...
	// Create the OSC 52 escape sequence
	...
	if (process.env['TMUX'] || process.env['STY']) {
		// Wrap in tmux/screen passthrough: ESC P tmux; ESC <osc52> ESC \
		osc52 = '\x1bPtmux;\x1b' + osc52 + '\x1b\\';
	}

	this.writeToOuput(osc52);
 ...
 */
JanewayClass.setMethod(function addIndicator(icon, options) {

	if (!icon) {
		throw new Error('A valid icon or option object is required');
	}
	...
	options = Object.assign({}, this.default_indicator_options, options);
 ...
 * @version  0.3.6
 */
JanewayClass.setMethod(function reloadScreen() {
...
JanewayClass.setMethod(function print(type, args, options) {
	...
	if (!this.logList || this.options.output_to_stdout) {
		...
		// Determine if we should use compact output (no colors, summarized objects)
		// Compact mode is used when not in a TTY (e.g., piped output, LLM context)
		let use_compact = !process.stdout.isTTY || process.env.JANEWAY_COMPACT;
		let use_colors = !use_compact && (process.env.COLORTERM || this.options.keep_color);

		if (use_compact) {
			// Only show type prefix for non-info messages (errors, warnings)
			let type_prefix = (type === 'info' || type === 'log') ? '' : '[' + type + '] ';
			trace = type_prefix + info.file + ':' + info.line + '  ';
		} else {
		...
		// Configure inspect options based on mode
		let inspect_options = use_compact
			? {colors: false, depth: 1, maxArrayLength: 3, maxStringLength: 100, breakLength: Infinity}
			: {colors: use_colors};

		for (; i < args.length; i++) {

			if (args[i] && typeof args[i] != 'string') {
				args[i] = util.inspect(args[i], inspect_options);
			}

			output += ' ';

			if (typeof args[i] != 'string') {
				output += util.inspect(args[i], inspect_options);
			} else if (use_compact) {
				// Strip escape codes from strings in compact mode
				output += stripEsc(args[i]);
			} else {
				output += args[i];
			}
		}

		// Remove colours when terminal doesn't support them (only needed in non-compact mode)
		if (!use_compact && !use_colors) {
...
JanewayClass.setMethod(function start(options, callback) {
	...
	if (options.customEval && typeof options.customEval === 'function') {
		...
		this._customEval = function(code) {
			return eval(code);
		};
	}
	...
	// Get the target stdout
	// (this is probably process.stdout, with its original .write method)
	let target_stdout = outputs.stdout;

	// If an extra output is given, all raw sequences have to be sent there too
	if (options.extra_output) {
		// Get the original write method
		const original_write = target_stdout.write;

		// Create an overlay object
		target_stdout = Object.create(target_stdout);

		// Overwrite the write method to also send it to the other output!
		target_stdout.write = function write(...args) {
			options.extra_output.write(...args);
			return original_write.apply(this, args);
		};
	}

	// Get the program instance
	this.program = blessed.program({
		output: target_stdout,
	});
		...
		if (listeners.length == 0) {
			process.on('uncaughtException', function onException(err) {
				...
				if (that.shutdown_on_exception) {
					...
					that.print('error', [err.message]);
					that.print('error', [err.stack]);
	...
	output.on('mouse', function onMouse(e) {
		...
		if (e.action == 'wheelup') {
			output.scroll(0 - that.config.scroll.step);
			scrolledUp = true;
			scrolled = true;
		} else if (e.action == 'wheeldown') {
			output.scroll(that.config.scroll.step);
			scrolledUp = false;
			scrolled = true;
		}
	...
	output.on('click', function onClick(data) {
		...
		let scroll = that.logList.box.childBase,
	...
	this.left_indicator_area = this.createIndicatorArea('left', {left: 0});
	this.center_indicator_area = this.createIndicatorArea('center');
	this.right_indicator_area = this.createIndicatorArea('right', {right: 0});
	this.indicator_area = this.right_indicator_area;
 ...
 * @version  0.4.2
 */
JanewayClass.setMethod(function _createMainMenu() {

	let menu_indicator = this.addIndicator(' ☰ ', {
		name  : 'main_menu',
		area  : 'left',
	});

	this.main_menu_indicator = menu_indicator;

	this.main_menu_indicator.addItem('Quit', {weight: -Infinity}, () => {
		process.exit();
	});

	if (!this.config.copy_json || !this.config.copy_json.enabled) {
		return;
	}

	this.copy_json_menu_item = menu_indicator.addItem(this.config.copy_json.title, () => {
		this.copyLineToJson();
	});

	this.main_menu_indicator.scheduleSort();
});

/**
 * Copy the current selected line to JSON
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.4.2
 * @version  0.4.2
 */
JanewayClass.setMethod(function copyLineToJson() {

	let selection = this.logList.current_selection,
	    line = this.logList.current_selected_line,
	    json,
	    temp;
	...
	if (this.config.copy_json.try_clipboard) {
		// Try to copy using OSC 52 escape sequence (works over SSH)
		copied_to_clipboard = this.writeToClipboard(json);
	}

	if (this.config.copy_json.dump_to_file) {

		let path;

		if (typeof this.config.copy_json.dump_to_file == 'string') {
			path = this.config.copy_json.dump_to_file;
		} else {
			path = libpath.resolve(this.user_janeway_path, 'clipboard.json');
		}

		fs.writeFile(path, json, (err) => {
			if (err) {
				this.printError('Failed to dump to file', err);
			} else if (copied_to_clipboard) {
				this.setStatus('Copied to clipboard and dumped to file');
			} else {
				this.setStatus('Dumped to file at ' + path);
			}
		});
	} else if (copied_to_clipboard) {
 ...
 * @version  0.3.6
 */
JanewayClass.setMethod(function printError(message, error) {

	let line = '{#ff5000-fg}ERROR: {/#ff5000-fg}';
 ...
 * @version  0.4.2
 */
JanewayClass.setMethod(function createIndicatorArea(name, options) {
...
/**
 * Render a specific indicator area
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.2.1
 * @version  0.4.2
 */
JanewayClass.setMethod(function _renderIndicatorArea(area) {

	if (!area) {
		area = this.indicator_area;
	} else if (typeof area == 'string') {
		area = this[area + '_indicator_area'];
	}
	...
	let is_left = area == this.left_indicator_area,
	    is_center = area == this.center_indicator_area,
	    is_right = area == this.right_indicator_area,
	    property;
	...
	// Sort the indicators
	Blast.Bound.Array.sortByPath(area.indicators, 'weight', 'creation_number');
	
	if (is_left || is_center) {
		area.indicators.reverse();
	}
	...
	for (i = 0; i < area.indicators.length; i++) {
		indicator = area.indicators[i];

		indicator.box.position[property] = value;
		value += indicator.width;
	}

	if (is_center) {
		area.left = Math.floor(this.screen_width / 2) - Math.floor(value / 2);
	}
...
Blast.require(['class', 'indicator_item'], {pwd: __dirname});
