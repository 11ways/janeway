...
JanewayClass.setMethod(function start(options, callback) {
	...
	// Create the "Copy Json" menu button (if it's enabled)
	this._createCopyJsonButton();
 ...
 * @version  0.4.1
 */
JanewayClass.setMethod(function _createCopyJsonButton() {

	if (!this.config.copy_json || !this.config.copy_json.enabled) {
		return;
	}

	let button_copy = blessed.box({
		parent    : this.menu,
		mouse     : true,
		autoFocus : false,
		name      : 'copy',
		content   : this.config.copy_json.title,
		shrink    : true,
		padding   : {
			left  : 1,
			right : 1
	...
	button_copy.on('click', () => this.copyLineToJson());
});
