...
JanewayClass.setMethod(function _createMenu() {
	...
	button_copy.on('click', function pressedCopy(data) {
		...
		if (that.config.copy_json.dump_to_file) {
			...
			if (typeof that.config.copy_json.dump_to_file == 'string') {
				path = that.config.copy_json.dump_to_file;
			} else {
				path = libpath.resolve(that.user_janeway_path, 'clipboard.json');
			}

			fs.writeFile(path, json, (err) => {
				if (err) {
					that.printError('Failed to dump to file', err);
				} else {
					that.setStatus('Dumped to file at ' + path);
				}
			});
		}
